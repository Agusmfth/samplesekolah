document.addEventListener('DOMContentLoaded', () => {
  ['#akademik','#guru','#fasilitas','#berita'].forEach((selector) => {
    ...
    cards.forEach((card, index) => {
      const fields = [...card.querySelectorAll('input:not([type=hidden]):not([type=file]), textarea')]; const name = fields[0]?.value || `Belum diisi (${index + 1})`; const info = fields[1]?.value || '-';
